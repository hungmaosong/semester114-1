...
uint32_t Block::getValidPageCountInEdgeBlock(uint64_t lpnBegin, uint64_t lpnEnd) {
  uint32_t ret = 0;

  if (ioUnitInPage == 1) {
    for(uint32_t i = 0 ; i < pValidBits->size(); i++){
      if(pValidBits->test(i)){
        if(pLPNs[i] >= lpnBegin && pLPNs[i] < lpnEnd){
          ret = ret + 1;
        }
      }
    }
...
bool Block::pageIsValid(uint32_t pageIndex) {
  bool valid = false;
  valid = pValidBits->test(pageIndex);
  return valid;
}

void Block::invalidateBlock() {
  pValidBits->reset();
}


/*
idx 和 ioUnitInPage 因為不會用到，可以直接默認 idx = 0，ioUnitInPage = 1

可能會使用到的參數:
假如一個 block 有 32 個 page
pValidBits : 長度為 32 的 bitset，分別對應 offset 為 0 ~ 31 的 page，初始所有 bit 為 0，當有 page 被寫入之後，相應的 bit 會變成 1，當被 invalidate 的時候會變成0
pErasedBits : 同上，但初始所有 bit 為 1，被寫入之後相應的 bit 變為 0，當 GC 觸發 erase 的時候才會將 pErasedBits 重置回全部為 1 的狀態 ( 只有對應的 bit 為 1 的時候才能夠寫入這個 page！)
*/ 

bool Block::nothingOnThisPage(uint32_t pageIndex) {
  
  // 回傳 page 是否為乾淨可以寫入的(erased)
  bool nothing = false;
  ...
  // 作業假設 ioUnitInPage == 1，只用單一路徑
  if (pErasedBits == nullptr || pValidBits == nullptr) {
    // 若尚未初始化，保守地回 false（避免崩潰）
    return false;
  }

  // 「可寫」＝ 已抹除(=1) 且 目前沒有資料(=0)
  nothing = pErasedBits->test(pageIndex) && !pValidBits->test(pageIndex);
  return nothing;

}

std::vector<int> Block::getValidPageList() {
  // 回傳一個 vector，裡面包含所有 validPage 的 offset
  std::vector<int> pages;
  if (pageCount == 0) return pages;

  pages.reserve(pageCount);

  // 作業假設 ioUnitInPage == 1；直接用 pValidBits 判斷
  if (pValidBits == nullptr) return pages;  // 尚未初始化就不搬
...
void Block::getLBN(uint32_t pageIndex, uint64_t &lbn) {
